...
| `MINIBLUE_DISABLE_SHAREDKEY_AUTH` | _(unset)_ | When truthy (`1`, `true`, `yes`, `on`), disables SharedKey signature verification on the blob data-plane. Dev-only — never enable in environments that model production. |
